...
static void paging_test_allocate_test_pages() {
    serial_printf(LOG_INFO "Allocating Physical Pages for Paging Test\n");
...
static void paging_test_map_and_init(paddr_t as, paddr_t *phys, uint64_t pattern) {
    serial_printf(LOG_INFO "Paging: mapping and initializing address space\n");
...
static void paging_test_verify(paddr_t as, paddr_t *phys, uint64_t pattern) {
    serial_printf(LOG_INFO "Paging: verifying address space\n");
    paging_switch_address_space(as);
    for (size_t i = 0; i < PAGING_TEST_PAGES; i++) {
        void *vaddr = paddr_to_vaddr(phys[i]);
...
static void paging_test_cr3_stress_test() {
    serial_printf(LOG_INFO "Paging: starting CR3 stress test (10000 switches)\n");
...
static void paging_test_cleanup() {
    serial_printf(LOG_INFO "Paging: cleaning up test address spaces and pages\n");
...
void paging_test_self_test(void) {
    serial_printf(LOG_INFO "Paging: self-test begin\n");
    ...
    serial_printf(LOG_INFO "Created address spaces: SEC1=%p, SEC2=%p\n", (void*)SEC1, (void*)SEC2);
    ...
    serial_printf(LOG_INFO "Kernel mapping verified\n");

    paging_test_cr3_stress_test();
    paging_test_cleanup();

    serial_printf(LOG_OK "Paging Test Passed!\n");
}
